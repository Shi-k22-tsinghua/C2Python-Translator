...
# 将 AST 转换为 YAML 格式并写入文件
def ast_to_yaml(ast, file_path):
    yaml = YAML()
    with open(file_path, 'w') as outfile:
        yaml.dump(ast.to_yaml_dict(), outfile)
...
def test_lex():
    try:
        ...
        # print(result)
        # 将结果写入 YAML 文件
        ast_to_yaml(result, 'ast.yaml')
        print("AST has been written to 'ast.yaml'")
    except EOFError:
